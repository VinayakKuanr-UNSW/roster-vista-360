...
const MonthView: React.FC<MonthViewProps> = ({
  ...
  const getDayContent = (day: Date) => {
    ...
    return (
      <div className={`
        ...
        ${isCurrentDay 
          ? 'border-blue-400 bg-blue-500/10 shadow-lg ring-1 ring-blue-400/30' 
          : isCurrentMonth 
      ...
      `}>
        ...
        <div className={`
          ...
          ${isCurrentDay
            ...
            : 'bg-gradient-to-r from-fuchsia-700/80 to-purple-700/80 border-white/10'
          }
        `}>
          <div className="flex items-center space-x-2">
            ...
            {isCurrentDay && (
              <div className="text-xs bg-blue-500/80 text-white px-2 py-0.5 rounded-full font-medium animate-pulse">
                Today
              </div>
            )}
          </div>
        ...
        <div className="flex-1 p-1 overflow-hidden flex flex-col">
          {shifts.length > 0 ? (
            <div className="flex-1 flex flex-col gap-1">
              {groupedShifts.slice(0, 2).map(([role, roleShifts], index) => (
                <div key={index} className="flex-1 min-h-0">
                  <div className={`
                    ...
                    ${roleShifts[0].groupColor === 'blue' 
                      ? 'from-blue-500/90 to-blue-600/90 hover:from-blue-500 hover:to-blue-600 border-blue-400/30' 
                      : roleShifts[0].groupColor === 'green'
                        ? 'from-green-500/90 to-green-600/90 hover:from-green-500 hover:to-green-600 border-green-400/30'
                        : roleShifts[0].groupColor === 'red'
                          ? 'from-red-500/90 to-red-600/90 hover:from-red-500 hover:to-red-600 border-red-400/30'
                          : 'from-purple-500/90 to-purple-600/90 hover:from-purple-500 hover:to-purple-600 border-purple-400/30'
                    }
                  `}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {isAMShift(roleShifts[0].shift) ? (
                        <Sun className="w-3 h-3 text-yellow-300 flex-shrink-0" />
                      ) : (
                        <Moon className="w-3 h-3 text-blue-300 flex-shrink-0" />
                      )}
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      {roleShifts.length > 1 && (
                        <div className="text-[10px] opacity-75 bg-white/20 px-1 rounded">
                          +{roleShifts.length - 1}
                        </div>
                      )}
                    </div>
  ...
  return (
    <div className="h-full bg-gradient-to-br from-gray-900 to-gray-800 rounded-lg border border-gray-700 overflow-hidden flex flex-col shadow-xl">
      ...
      <div className="flex-1 overflow-auto flex flex-col">
        ...
        <div className="grid grid-cols-7 border-b border-gray-700 bg-gray-800/80 flex-shrink-0">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div key={day} className="p-3 text-center text-sm font-semibold text-white/90 border-r border-gray-700 last:border-r-0 bg-gradient-to-b from-gray-700 to-gray-800">
              {day}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 flex flex-col">
          {weeks.map((week, weekIndex) => (
            <div key={weekIndex} className="flex-1 grid grid-cols-7 border-b border-gray-700 last:border-b-0">
              {week.map((day, dayIndex) => (
                <div key={dayIndex} className="border-r border-gray-700 last:border-r-0 flex">
                  {getDayContent(day)}
                </div>
              ))}
            </div>
