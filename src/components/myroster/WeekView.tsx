...
const WeekView: React.FC<WeekViewProps> = ({
  ...
  return <div className="h-full bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
      {/* Header with days */}
      <div className="grid grid-cols-[80px_repeat(7,1fr)] border-b border-gray-800 bg-gray-800/50">
        <div className="p-3"></div>
        {days.map((day, i) => <div key={i} className={`p-3 text-center border-l border-gray-700 ${isToday(day) ? 'bg-blue-900/20' : ''}`}>
            <div className="text-sm font-XL">{format(day, 'EEE')}</div>
            <div className="text-xs text-gray-400">{format(day, 'd')}</div>
            {isToday(day) && <div className="mt-1 inline-block bg-blue-500/30 text-blue-200 text-xs px-1.5 py-0.5 rounded">
                Today
              </div>}
          </div>)}
      </div>
      ...
      <div className="relative h-[calc(100%-4rem)] overflow-y-auto">
        ...
        <div className="grid grid-cols-[80px_repeat(7,1fr)]">
          {hours.map(hour => <React.Fragment key={hour}>
              ...
              {days.map((_, dayIndex) => <div key={dayIndex} className="h-16 border-b border-gray-800 border-l border-gray-700"></div>)}
            </React.Fragment>)}
        </div>
        ...
        <div className="absolute top-0 left-20 right-0 bottom-0 grid grid-cols-7 gap-px">
          ...
          return <div key={dayIndex} className="relative">
              ...
              return <div key={shiftIndex} className="absolute left-1 right-1" style={{
                top: `${startPos}%`,
                height: `${Math.max(height, 3)}%`,
                minHeight: '32px'
              }}>
                      <MyRosterShift shift={shiftData.shift} groupName={shiftData.groupName} groupColor={shiftData.groupColor} subGroupName={shiftData.subGroupName} onClick={() => setSelectedShift(shiftData)} style={{
                  height: '100%'
                }} compact={true} />
                    </div>;
            })}
